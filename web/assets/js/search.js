...
//   const products = document.querySelectorAll(".onian,.shopApple,.img_1");

//   products.forEach(product => {
//     const productName = product.querySelector("h3,h2").innerText.toLowerCase();
...
//   toggleCart();          // re-render the cart list
// }

const PRODUCTS = [/* your product data */];

// Live dropdown as user types
...
// Full search on button click or Enter
function handleSearch() {
  const q = searchInput.value.trim().toLowerCase();
  const matches = PRODUCTS.filter(p => p.name.toLowerCase().includes(q));
  renderProducts(matches); // re-render your product grid
}